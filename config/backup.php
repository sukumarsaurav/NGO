...
return [
    ...
    'backup' => [
        ...
        'destination' => [
             ...
             *
             * `backup-offsite` is only included once its bucket is actually
             * configured — Spatie constructs every disk in this list eagerly
             * (even before `continue_on_failure` can apply), so an S3 disk
             * with a null bucket crashes the whole command instead of just
             * failing that one target. Safe to add unconditionally once
             * Sprint 16 provisions BACKUP_AWS_* in production.
             */
            'disks' => array_filter([
                'local',
                env('BACKUP_AWS_BUCKET') ? 'backup-offsite' : null,
            ]),

            /*
             * Determines whether to allow backups to continue when some targets fail instead of failing completely.
             *
             * Kept true as defense-in-depth even though the `disks` list above
             * already excludes `backup-offsite` until it's configured — if
             * that disk starts failing for a real reason after Sprint 16
             * provisions it (bad credentials, network outage), the local
             * copy should still succeed rather than the whole nightly run
             * failing.
             */
    ...
    'notifications' => [
        ...
        'mail' => [
            // Config loads before the DB-backed org settings are available,
            // so this can't read `org.email` — falls back to MAIL_FROM_ADDRESS
            // (already required for the app to send any mail at all) rather
